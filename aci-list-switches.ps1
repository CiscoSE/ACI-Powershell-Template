...
function getAllLeafSwitches{
    ...
    $listOfSwitches.ChildNodes.fabricNode |
        ?{$_.role -match "leaf|spine"} | 
            select id,address,name,model,status,role,dn |
                Sort-Object role, id |
                    ft -AutoSize
    #$listOfSwitches.ChildNodes.fabricNode | get-member -MemberType Properties
}
